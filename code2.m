...
%             waitbar(((j-1)*N+i)/(length(tt)*N),hwait,'已经完成...')
    ...
    title({['t=',num2str(300+step*dt)],'','继续盘入，轨迹'})
    drawnow
...
% 创建表头
headers = {'X (m)', 'Y (m)', 'Velocity (m/s)'};
writecell(headers, filename, 'Sheet', 1, 'Range', 'A1');

% 写入数据
writematrix(X(:,end), filename, 'Sheet', 1, 'Range', 'A2');
writematrix(Y(:,end), filename, 'Sheet', 1, 'Range', 'B2');
writematrix(V, filename, 'Sheet', 1, 'Range', 'C2');
